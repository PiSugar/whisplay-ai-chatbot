...
let summaryTextWithLLM: SummaryTextWithLLMFunction = noop as any;
...
switch (llmServer) {
  case LLMServer.volcengine:
    ({ chatWithLLMStream, resetChatHistory } = volcengineLLM); 
    break;
  case LLMServer.openai:
    ({ chatWithLLMStream, resetChatHistory } = openaiLLM);
    break;
  case LLMServer.ollama:
    ({ chatWithLLMStream, resetChatHistory, summaryTextWithLLM } = ollamaLLM);
    break;
  case LLMServer.gemini:
    ({ chatWithLLMStream, resetChatHistory } = geminiLLM);
    break;
  case LLMServer.grok:
    ({ chatWithLLMStream, resetChatHistory } = grokLLM);
    break;
